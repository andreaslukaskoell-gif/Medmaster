...
- Scan the `content` field for markdown blockquotes with syntax: `> **MerksatzBox type="merke":** {text}`
- Extract 5-7 most important merke/altfragen items per Unterkapitel
- Populate the `merksätze: []` field with clean text (remove markdown syntax)
- Pattern: `merksätze: ['text1', 'text2', ...]` (following kap7 structure)

**2. Extract klinischerBezug field:**
- Scan for `> **MerksatzBox type="klinisch":** {text}` in content
- Extract the clinical highlight text and create `klinischerBezug` field in Unterkapitel object
- Pattern: `klinischerBezug: 'clinical text here'` (following kap7/kap8 structure)
- If multiple klinisch blocks exist, combine or use the most comprehensive one

**3. Extract altfrage field:**
- Scan for `> **MerksatzBox type="altfragen":** {text}` in content
- Extract exam-relevant text and create `altfrage` field with structure: `{ question: 'text', answer: 'brief answer' }`
- Pattern follows kap7/kap8: altfrage is optional but should capture key exam hint if present
