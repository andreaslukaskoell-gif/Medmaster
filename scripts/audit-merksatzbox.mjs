...
async function auditMerksatzBox() {
  ...
  for (const [chapterId, stats] of Object.entries(chapterStats)) {
    ...
    const status = stats.totalBoxes >= 2 ? '✅ Compliant' : stats.totalBoxes === 1 ? '⚠️ Deficit' : '❌ Missing';
  ...
  console.log(`✅ Compliant (2+ boxes): ${totalCompliant} (${(totalCompliant/totalChapters*100).toFixed(1)}%)`);
  console.log(`⚠️ Deficit (1 box): ${totalDeficit} (${(totalDeficit/totalChapters*100).toFixed(1)}%)`);
  console.log(`❌ Missing (0 boxes): ${totalMissing} (${(totalMissing/totalChapters*100).toFixed(1)}%)`);
  console.log();
