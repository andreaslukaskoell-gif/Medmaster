...
function sectorPath(cx, cy, r, a1, a2) {
  const x1 = rd(cx + r * Math.cos(a1)),
    y1 = rd(cy + r * Math.sin(a1));
  const x2 = rd(cx + r * Math.cos(a2)),
    y2 = rd(cy + r * Math.sin(a2));
...
function makePolygonPieces(n, numPieces, rots) {
  ...
  const perG = Math.floor(n / numPieces),
    extra = n % numPieces;
  const pieces = [];
  let idx = 0;
...
const SPECS = [
  ...
  },
  {
    ...
    diff: "mittel",
    gen: () =>
      makeCircleSectors(5, 2 * PI, 0, [PI / 5, (3 * PI) / 5, PI, (7 * PI) / 5, (9 * PI) / 5]),
  },
  {
    id: "fz-24",
    correct: "trapez",
  ...
  },
  {
    ...
    diff: "schwer",
    gen: () =>
      makeCircleSectors(6, (3 * PI) / 2, PI / 4, [
        PI / 8,
        (3 * PI) / 8,
        (5 * PI) / 8,
        (7 * PI) / 8,
  ...
  },
  {
    ...
    gen: () => {
      ...
      const pieces = [
        [center, verts[0], mid01],
        [center, mid01, verts[1]],
        [center, verts[1], verts[2]],
        [center, verts[2], verts[3]],
        [center, verts[3], verts[4]],
        [center, verts[4], verts[0]],
      ];
