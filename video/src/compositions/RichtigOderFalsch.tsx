    ...
    <BrandedBackground>
      <SafeArea style={{ alignItems: "center", justifyContent: "center", gap: 24 }}>
        ...
        <div style={{
          ...
          <div style={{ padding: "16px 36px", borderRadius: 18, background: "rgba(34,197,94,0.2)", border: `2px solid ${BRAND.success}`, fontSize: 32, color: "#4ade80", fontWeight: 700 }}>✅ Richtig</div>
          <div style={{ padding: "16px 36px", borderRadius: 18, background: "rgba(239,68,68,0.2)", border: `2px solid ${BRAND.error}`, fontSize: 32, color: "#f87171", fontWeight: 700 }}>❌ Falsch</div>
        </div>
        <div style={{ fontSize: 28, color: BRAND.textMuted, marginTop: 20 }}>Kommentiere ✅ oder ❌ bei jeder!</div>
      </SafeArea>
    </BrandedBackground>
  );
};
...
const DifficultyBadge: React.FC<{ level: number }> = ({ level }) => {
  ...
  return (
    <div style={{ padding: "8px 20px", borderRadius: 12, background: `${colors[level]}22`, border: `2px solid ${colors[level]}`, fontSize: 22, color: colors[level], fontWeight: 700 }}>
      {labels[level]}
    </div>
  );
};
  ...
  <BrandedBackground>
    <SafeArea style={{ alignItems: "center", justifyContent: "center", gap: 24 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <div style={{ fontSize: 22, color: "rgba(255,255,255,0.4)", fontWeight: 600 }}>RICHTIG ODER FALSCH?</div>
        <DifficultyBadge level={index} />
      </div>
      <div style={{
        padding: "44px 36px", background: "rgba(255,255,255,0.06)", borderRadius: 28,
        border: "2px solid rgba(255,255,255,0.15)", width: "100%",
      }}>
        <AnimatedText text={`\u201E${text}\u201C`} fontSize={44} fontWeight={700} delay={3} lineHeight={1.4} style={{ textShadow: "0 2px 6px rgba(0,0,0,0.3)" }} />
      </div>
      <AnimatedText text="🤔 Was denkst du?" fontSize={32} color="rgba(255,255,255,0.3)" delay={15} />
    </SafeArea>
  </BrandedBackground>
);
  ...
  return (
    <BrandedBackground>
      <SafeArea style={{ alignItems: "center", justifyContent: "center", gap: 20 }}>
        ...
        <div style={{ fontSize: 60, fontWeight: 900, color, textShadow: "0 4px 12px rgba(0,0,0,0.4)" }}>{label}</div>
        <div style={{ padding: 36, background: "rgba(255,255,255,0.06)", borderRadius: 22, width: "100%" }}>
          <div style={{ fontSize: 36, color: "white", textAlign: "center", lineHeight: 1.4 }}>{`\u201E${text}\u201C`}</div>
        </div>
      </SafeArea>
    </BrandedBackground>
  );
...
export const RichtigOderFalsch: React.FC<RichtigOderFalschProps> = ({ statements }) => {
  ...
  return (
    <AbsoluteFill>
      <Sequence from={0} durationInFrames={introEnd}><IntroScene /></Sequence>
      {stmts.map((s, i) => (
