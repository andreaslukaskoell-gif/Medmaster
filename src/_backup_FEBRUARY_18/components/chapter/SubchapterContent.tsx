...
function formatTextEnhanced(text: string): string {
  ...
  t = t.replace(
    /^### (.*?)$/gm,
    '<h4 class="text-base font-semibold text-gray-800 dark:text-gray-200 mt-4 mb-2">$1</h4>'
  );
  t = t.replace(
    /^## (.*?)$/gm,
    '<h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mt-6 mb-3 border-b border-gray-200 dark:border-gray-700 pb-2">$1</h3>'
  );
  t = t.replace(
    /^# (.*?)$/gm,
    '<h2 class="text-xl font-bold text-gray-900 dark:text-gray-100 mt-8 mb-4">$1</h2>'
  );
  ...
  t = t.replace(
    /^> (.*?)$/gm,
    '<blockquote class="border-l-4 border-amber-400 bg-amber-50 dark:bg-amber-900/20 pl-4 py-2 my-3 rounded-r">$1</blockquote>'
  );
          ...
          <div key={i} className={`space-y-4 ${isDieZelle ? "mb-6" : "space-y-3"}`}>
            ...
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: isDieZelle ? formatTextEnhanced(section.text) : formatText(section.text),
              }}
            />
            ...
            {section.merksatz && (
              ...
              >
                ...
                <p
                  ...
                  dangerouslySetInnerHTML={{
                    __html: isDieZelle
                      ? formatTextEnhanced(section.merksatz)
                      : formatText(section.merksatz),
                  }}
                />
              </div>
            )}
  ...
  return (
    <div className={`${isDieZelle ? "space-y-8" : "space-y-6"}`}>
      <div
        ...
        dangerouslySetInnerHTML={{
          __html: isDieZelle
            ? formatTextEnhanced(cleanedContent)
            : formatText(cleanedContent, true),
        }}
      />
      {uk.diagram && (
