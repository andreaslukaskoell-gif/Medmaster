...
import { Copy, Check, FileText, Eye, Save, Clock, History, ArrowRight, Plus, X } from 'lucide-react';
...
const kapitelListe = [
  { path: 'biologie/kap1-zelle', label: 'Biologie - Kap 1: Die Zelle', subject: 'biologie' as const },
  { path: 'biologie/kap2-koerper', label: 'Biologie - Kap 2: Körper', subject: 'biologie' as const },
  { path: 'biologie/kap3-fortpflanzung', label: 'Biologie - Kap 3: Fortpflanzung', subject: 'biologie' as const },
  { path: 'biologie/kap4-genetik', label: 'Biologie - Kap 4: Genetik', subject: 'biologie' as const },
  { path: 'biologie/kap5-molekulargenetik', label: 'Biologie - Kap 5: Molekulargenetik', subject: 'biologie' as const },
  { path: 'biologie/kap6-evolution', label: 'Biologie - Kap 6: Evolution', subject: 'biologie' as const },
  { path: 'biologie/kap7-oekologie', label: 'Biologie - Kap 7: Ökologie', subject: 'biologie' as const },
  { path: 'biologie/kap8-immunbiologie', label: 'Biologie - Kap 8: Immunbiologie', subject: 'biologie' as const },
  { path: 'chemie/kap1-periodensystem', label: 'Chemie - Kap 1: Periodensystem', subject: 'chemie' as const },
  ...
  { path: 'chemie/kap4-stoechiometrie', label: 'Chemie - Kap 4: Stöchiometrie', subject: 'chemie' as const },
  { path: 'chemie/kap5-gasgesetze', label: 'Chemie - Kap 5: Gasgesetze', subject: 'chemie' as const },
  { path: 'chemie/kap6-aggregatzustaende', label: 'Chemie - Kap 6: Aggregatzustände', subject: 'chemie' as const },
  { path: 'chemie/kap7-reaktionen', label: 'Chemie - Kap 7: Reaktionen', subject: 'chemie' as const },
  { path: 'chemie/kap8-saeure-base', label: 'Chemie - Kap 8: Säure-Base', subject: 'chemie' as const },
  ...
  { path: 'chemie/kap11-biochemie', label: 'Chemie - Kap 11: Biochemie', subject: 'chemie' as const },
  { path: 'chemie/kap12-stoffwechsel', label: 'Chemie - Kap 12: Stoffwechsel', subject: 'chemie' as const },
  ...
  { path: 'physik/kap6-elektrizitaet', label: 'Physik - Kap 6: Elektrizität', subject: 'physik' as const },
  { path: 'physik/kap7-wellen', label: 'Physik - Kap 7: Wellen', subject: 'physik' as const },
  { path: 'physik/kap8-atomphysik', label: 'Physik - Kap 8: Atomphysik', subject: 'physik' as const },
  { path: 'mathematik/kap1-grundlagen', label: 'Mathematik - Kap 1: Grundlagen', subject: 'mathematik' as const },
  { path: 'mathematik/kap2-algebra', label: 'Mathematik - Kap 2: Algebra', subject: 'mathematik' as const },
  { path: 'mathematik/kap3-funktionen', label: 'Mathematik - Kap 3: Funktionen', subject: 'mathematik' as const },
  { path: 'mathematik/kap4-trigonometrie', label: 'Mathematik - Kap 4: Trigonometrie', subject: 'mathematik' as const },
  { path: 'mathematik/kap5-analysis', label: 'Mathematik - Kap 5: Analysis', subject: 'mathematik' as const },
  { path: 'mathematik/kap6-geometrie', label: 'Mathematik - Kap 6: Geometrie', subject: 'mathematik' as const },
];
...
function formatText(text: string, escapeHtml = false, subjectBorderColor?: string, subjectBgColor?: string): string {
  let t = text;
  if (escapeHtml) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (headingMatch) {
      ...
      processed.push(`<h${level} class="${headingClasses[level as keyof typeof headingClasses] || ''}">${content}</h${level}>`);
      continue;
    }
...
export default function KapitelEditor() {
  ...
  const [detectedSubject, setDetectedSubject] = useState<'biologie' | 'chemie' | 'physik' | 'mathematik'>('biologie');
  ...
  const detectSubject = (text: string): 'biologie' | 'chemie' | 'physik' | 'mathematik' => {
    ...
    if (lower.includes('zelle') || lower.includes('dna') || lower.includes('genetik') || lower.includes('evolution') || lower.includes('biologie')) {
      return 'biologie';
    }
    if (lower.includes('periodensystem') || lower.includes('atom') || lower.includes('bindung') || lower.includes('reaktion') || lower.includes('chemie')) {
      return 'chemie';
    }
    if (lower.includes('kinematik') || lower.includes('mechanik') || lower.includes('elektrizität') || lower.includes('welle') || lower.includes('physik')) {
      return 'physik';
    }
    if (lower.includes('algebra') || lower.includes('analysis') || lower.includes('geometrie') || lower.includes('trigonometrie') || lower.includes('mathematik')) {
      return 'mathematik';
    }
  ...
  const checkSubchapterExists = (chapterId: string, subchapterTitle: string): boolean => {
    const chapter = loadChapterById(chapterId) || allAvailableChapters.find(c => c.id === chapterId);
    if (!chapter || !chapter.unterkapitel) return false;
    
    return chapter.unterkapitel.some(uk => 
      uk.title.toLowerCase().trim() === subchapterTitle.toLowerCase().trim()
    );
  };
  ...
  const generateUniqueFilename = async (
    ...
    try {
      ...
      if (counter > maxAttempts) {
        console.error('Konnte keinen eindeutigen Dateinamen generieren nach', maxAttempts, 'Versuchen');
  ...
  const createChapterFromPaste = async () => {
    ...
    if (isCreatingNewChapter) {
      ...
      const existingChapter = chaptersForSubject.find(ch => 
        ch.title.toLowerCase().trim() === newChapterTitle.trim().toLowerCase()
      );
      
      if (existingChapter) {
        alert(`⚠️ Ein Überkapitel mit dem Titel "${newChapterTitle}" existiert bereits!\n\nBitte wähle es aus dem Dropdown aus oder verwende einen anderen Titel.`);
        return;
      }
    ...
    if (!subchapterTitle || subchapterTitle.length < 3) {
      alert('⚠️ Die erste Zeile des Inhalts muss den Titel des Unterkapitels enthalten!\n\nBitte stelle sicher, dass die erste Zeile mindestens 3 Zeichen lang ist.');
      return;
    }
    ...
    try {
      ...
      if (isCreatingNewChapter) {
        ...
        // Existierendes Überkapitel verwenden
        const existingChapter = loadChapterById(chapterId) || chaptersForSubject.find(ch => ch.id === chapterId);
        if (!existingChapter) {
      ...
      const subchaptersCode = allSubchapters.map(uk => {
...
${subchaptersCode}
      ...
      for (const k of existingChapters) {
        ...
        if (match) {
          ...
          const titleSlug = k.title.toLowerCase().replace(/[^a-z0-9äöüß]+/g, '-').replace(/^-|-$/g, '');
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 p-6">
      <div>
        ...
        <p className="text-muted mt-2">Wähle ein Überkapitel aus und füge Unterkapitel-Inhalt ein</p>
      </div>
              ...
              >
                ...
                {chaptersForSubject.map(ch => (
                  <option key={ch.id} value={ch.id}>
                    {ch.title} {ch.unterkapitel?.length ? `(${ch.unterkapitel.length} Unterkapitel)` : ''}
                  </option>
                ))}
              </select>
