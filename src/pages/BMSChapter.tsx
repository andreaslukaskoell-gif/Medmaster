...
import { ArrowLeft, CheckCircle2, Bookmark, StickyNote, Target, Lightbulb, Stethoscope, ListChecks } from "lucide-react";
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      {chapter.icon ? (
        ...
        <>
          <div>
            <p className="text-sm text-primary-700 dark:text-primary-400 font-medium uppercase tracking-wide">{subjectLabels[chapter.subject]}</p>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mt-1">{chapter.title}</h1>
          </div>
          {chapter.overview && (
            <Card className="border-primary-200 dark:border-primary-800 bg-primary-50/50 dark:bg-primary-900/10">
              <CardContent className="p-5">
                <p className="text-sm text-primary-800 dark:text-primary-300 leading-relaxed">{chapter.overview}</p>
              </CardContent>
            </Card>
          )}
        </>
      ...
      {showNotes && (
        <Card className="border-yellow-200 dark:border-yellow-800">
          <CardContent className="p-4 space-y-3">
            ...
            />
            <Button size="sm" onClick={handleSaveNote}>Speichern</Button>
          </CardContent>
        </Card>
      )}
        ...
        {chapter.sections.map((section, i) => {
          ...
          <Card key={i}>
            <CardContent className="p-6 space-y-4">
              ...
              {section.merke && (
                <MerksatzBox text={section.merke} type="merke" />
              )}

              {section.altfrage && (
                <MerksatzBox text={section.altfrage} type="altfragen" />
              )}

              {section.klinik && (
                <MerksatzBox text={section.klinik} type="klinisch" />
              )}
            </CardContent>
          </Card>
          );
          ...
          <CardContent className="p-5 space-y-4">
            ...
            {selfTestQuestions.map((q, qi) => (
              <div key={q.id} className="space-y-2">
                <p className="text-sm font-medium text-gray-800 dark:text-gray-200">{qi + 1}. {q.text}</p>
                  ...
                  {q.options.map((opt) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {opt.text}
                        {isCorrect && <Badge variant="success" className="ml-2">Richtig</Badge>}
                        {isWrong && <Badge variant="danger" className="ml-2">Falsch</Badge>}
                      </button>
                    );
                  })}
                </div>
                {selfTestSubmitted && (
                  <p className="text-xs text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 p-2 rounded ml-4">{q.explanation}</p>
                )}
              </div>
            ))}
            {!selfTestSubmitted ? (
              ...
              <p className="text-sm font-medium text-primary-700 dark:text-primary-400">
                {selfTestQuestions.filter((q) => selfTestAnswers[q.id] === q.correctOptionId).length}/{selfTestQuestions.length} richtig
              </p>
            )}
          </CardContent>
        </Card>
