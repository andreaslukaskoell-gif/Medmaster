...
import {
  ArrowLeft, Trophy, RotateCcw, BookOpen,
  Loader2, AlertCircle, ChevronRight, Dumbbell,
  ToggleLeft, ToggleRight, Shuffle, AlertTriangle,
} from 'lucide-react';
...
function SelectionScreen({ onStart, userId }: {
  ...
  const toggleUK = (id: string) =>
    setSelectedUKs(prev => { const n = new Set(prev); n.has(id) ? n.delete(id) : n.add(id); return n; });
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5">
          ...
          <div className="grid grid-cols-2 gap-2">
            {(['trainer', 'simulation'] as TrainerMode[]).map(m => (
              <button key={m} onClick={() => setMode(m)}
                ...
                <div className="font-semibold">{m === 'trainer' ? '🎓 Trainer' : '⚡ Simulation'}</div>
                <div className="text-xs opacity-70">
                  {m === 'trainer' ? 'Schritt für Schritt + Erklärungen' : 'Wie echte MedAT-Prüfung'}
                </div>
              </button>
            ))}
          </div>
      ...
      <Card>
        <CardContent className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            {BIO_CHAPTERS.map(ch => (
              <button key={ch.id}
                onClick={() => { setSelectedChapter(ch.id); setSelectedUKs(new Set()); }}
        ...
        <Card>
          <CardContent className="p-4 space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">2. Unterkapitel</p>
              <div className="flex gap-2">
                <button onClick={() => setSelectedUKs(new Set(subchapters.map(s => s.id)))}
                  className="text-xs text-emerald-600 hover:underline cursor-pointer">Alle</button>
                <span className="text-muted-foreground text-xs">·</span>
                <button onClick={() => setSelectedUKs(new Set())}
                  className="text-xs text-muted-foreground hover:underline cursor-pointer">Keine</button>
              </div>
            </div>
            {loading ? (
                ...
                {subchapters.map(uk => {
                  ...
                  return (
                    <label key={uk.id}
                      ...
                      <input type="checkbox" checked={selectedUKs.has(uk.id)}
                        onChange={() => toggleUK(uk.id)}
                        className="rounded border-gray-300 text-emerald-600 cursor-pointer" />
                      <span className="text-sm flex-1 text-gray-700 dark:text-gray-300">{uk.title}</span>
                      {hasError && (
      ...
      {/* Interleaving toggle */}
      {selectedChapter && (
        <button onClick={toggleInterleaving}
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="text-left">
              <p className="text-sm font-medium text-gray-700 dark:text-gray-300">MedAT-Mix (Interleaving)</p>
              <p className="text-xs text-muted-foreground">Fragen aus allen UKs gemischt (opt-in)</p>
            </div>
          </div>
          {interleaving
...
function QuizScreen({ uk_ids, mode, userId, onFinish, onBack }: {
  uk_ids: string[];
  mode: TrainerMode;
  userId: string;
  ...
  const {
    loading, error,
    currentFrage, idx, progress, fragen, sessionDone,
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${DIFF_COLOR[currentFrage.schwierigkeit]}`}>
...
function ResultsScreen({ answers, onRestart, onBack }: {
  ...
  for (const a of answers) {
    ...
    byDiff[d].t++; if (a.correct) byDiff[d].c++;
  }
      ...
      <Card>
        <CardContent className="p-6 text-center">
          ...
          <div className="text-5xl font-bold text-gray-900 dark:text-gray-100 mb-1">
            {correct}<span className="text-2xl text-muted-foreground font-normal">/{total}</span>
          </div>
          <p className="text-lg text-emerald-600 dark:text-emerald-400 font-semibold">{pct}% richtig</p>
          ...
          {pct >= 70 && pct < 90 && <p className="text-sm font-bold text-emerald-600 mt-2">✅ Sehr gut!</p>}
          {pct < 70 && <p className="text-sm text-muted-foreground mt-2">Weitermachen – du schaffst das!</p>}
        </CardContent>
      </Card>

      {Object.keys(byDiff).length > 0 && (
        <div className="grid grid-cols-3 gap-3">
          {([1, 2, 3] as const).filter(d => byDiff[d]).map(d => (
            <Card key={d}><CardContent className="p-3 text-center">
              <p className={`text-xs font-semibold mb-1 ${DIFF_COLOR[d].split(' ').slice(1).join(' ')}`}>
                {DIFF_LABEL[d]}
              </p>
            ...
            <Card key={frage.id} className="border-l-4 border-l-red-400">
              <CardContent className="p-4 space-y-2">
                <div className="flex items-start justify-between gap-2">
                  <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{frage.stamm}</p>
                  <span className="text-[10px] shrink-0 px-1.5 py-0.5 rounded border border-border text-muted-foreground">Typ {frage.typ}</span>
                </div>
                {frage.typ === 'A' && frage.optionen && (
                  <div className="space-y-1">
                    {frage.optionen.map(opt => (
                      <div key={opt.key} className={`text-xs px-3 py-1.5 rounded-lg ${
                ...
                {frage.erklaerung && (
                  <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3">
                    <p className="text-xs text-blue-700 dark:text-blue-400 leading-relaxed">{frage.erklaerung}</p>
                  </div>
                )}
              </CardContent>
            </Card>
  ...
  return (
    <div className="p-4 pb-24 md:p-6">
      {screen === 'select' && (
        <SelectionScreen
          userId={userId}
          onStart={(ids, m) => { setSelectedIds(ids); setMode(m); setScreen('quiz'); }}
        />
      )}
      {screen === 'quiz' && (
        <QuizScreen
          uk_ids={selectedIds} mode={mode} userId={userId}
