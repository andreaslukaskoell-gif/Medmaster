...
export default function ImplikationenSimulation() {
  const [phase, setPhase] = useState<SimPhase>("start");
  const [tasks, setTasks] = useState<ImplikationTask[]>([]);
  ...
  const finishSimulation = useCallback(() => {
    // Save remaining unanswered tasks
    setResults((prev) => {
  ...
  const handleSubmitAnswer = useCallback(() => {
    ...
    if (currentIndex + 1 < TASK_COUNT) {
      ...
      // Last task — finish with this result included
      setResults((prev) => {
        const allResults = prev.length === TASK_COUNT ? prev : [...prev];
        // Already added above
        const correctCount = allResults.filter((r) => r.correct).length;
        recordSimulation({
          id: `sim-imp-${Date.now()}`,
  ...
  }, [selectedAnswer, tasks, currentIndex, timeLeft, recordSimulation]);
      ...
      <Card>
        <CardContent className="p-8 text-center space-y-6">
          ...
          <button
            ...
            className="px-8 py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-lg font-semibold transition-colors cursor-pointer"
          >
            Simulation starten
          </button>
        </CardContent>
    ...
    return (
      <div className="space-y-6">
        ...
        <Card>
          <CardContent className="p-6">
            <div className="text-center space-y-4">
              <div className="w-16 h-16 mx-auto rounded-2xl flex items-center justify-center bg-gradient-to-br from-purple-500 to-indigo-600">
        ...
        <div className="flex justify-center">
          <button
            onClick={startSimulation}
            className="flex items-center gap-2 px-6 py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-lg font-medium transition-colors cursor-pointer"
          >
      ...
      <Card>
        <CardContent className="p-6 space-y-5">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-5 py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-lg text-sm font-semibold disabled:opacity-40 disabled:cursor-not-allowed transition-colors cursor-pointer"
          >
