...
import { Brain, BookOpen, Dumbbell, Timer } from "lucide-react";
import { BreadcrumbNav } from "@/components/ui/breadcrumb-wrapper";
import { useKFFStore } from "@/stores/kffStore";
import ImplikationenLernen from "./ImplikationenLernen";
import ImplikationenUeben from "./ImplikationenUeben";
import ImplikationenSimulation from "./ImplikationenSimulation";
...
export default function ImplikationenPage() {
  const [activeTab, setActiveTab] = useState<TabKey>("lernen");
  const progress = useKFFStore((s) => s.progress.implikationen);
  const pct =
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-foreground">Implikationen erkennen</h1>
      ...
      {/* Tab content */}
      {activeTab === "lernen" && <ImplikationenLernen />}
      {activeTab === "ueben" && <ImplikationenUeben />}
      {activeTab === "simulation" && <ImplikationenSimulation />}
    </div>
  );
}
