...
export function ImageWithFallback({
  ...
  const placeholderBase =
    "absolute inset-0 flex items-center justify-center bg-linear-to-br from-muted to-[var(--accent)]/5 dark:from-muted dark:to-[var(--accent)]/30";
      ...
      >
        ...
        {status === "loading" && (
          <div className={`${placeholderBase} animate-pulse`} aria-hidden>
            <div className="h-16 w-16 rounded-full bg-[var(--surface)]/80 blur-md" />
            <ImageIcon className="absolute w-8 h-8 text-[var(--muted)]/70" />
          </div>
        )}
        ...
        {status === "error" && (
          <div className={`${placeholderBase} min-h-[120px]`}>
            <div className="text-center px-4">
              <ImageOff className="w-10 h-10 mx-auto text-[var(--muted)]/70 mb-2" />
              <p className="text-xs text-[var(--muted)]">Bild konnte nicht geladen werden</p>
            </div>
          </div>
        )}
