...
export function TopBar({ onMenuToggle }: { onMenuToggle?: () => void }) {
  ...
  return (
    <header className={`topbar-glass sticky top-0 z-[100] flex items-center justify-between gap-2 h-12 ${isMobile ? "px-3" : "px-6 gap-3"}`}>
      ...
      <div className="flex items-center gap-2 min-w-0 flex-1">
        ...
        {isMobile && (
          <span className="text-sm font-semibold text-[var(--text-primary)] truncate">MedMaster</span>
        )}
      </div>
      ...
      <div className={`flex items-center shrink-0 ${isMobile ? "gap-1" : "gap-3"}`}>
        ...
        {isMobile && (
          <button
            type="button"
            onClick={openCommandPalette}
            className="p-2 rounded-lg text-[var(--muted)] hover:text-[var(--foreground)] transition-colors cursor-pointer"
            aria-label="Suchen"
          >
            <Search className="w-4 h-4" />
          </button>
        ...
        <div className="flex items-center text-xs text-[var(--muted)]">
          <Tooltip content="Dein Level" position="bottom">
            <Link
              to="/fortschritt"
              className="flex items-center gap-1 px-1.5 py-1.5 rounded-lg hover:bg-[var(--foreground)]/5 hover:text-[var(--foreground)] transition-colors"
            >
          ...
          <Tooltip content="Tage-Streak" position="bottom">
            <div className="flex items-center gap-1 px-1.5 py-1.5 rounded-lg hover:bg-[var(--foreground)]/5 hover:text-[var(--foreground)] transition-colors">
              <StreakFlameIcon
                streak={streak}
