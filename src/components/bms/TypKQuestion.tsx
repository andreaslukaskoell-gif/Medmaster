...
export function TypKQuestion({
  frage, mode, confidence,
  typKPhase, typKDecisions, typKCombChosen, revealed,
  onJudge, onConfirmPhase1, onChooseCombination,
}: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ── AUSSAGEN ─────────────────────────────────────── */}
      <div className={`space-y-2 transition-opacity duration-200 ${locked ? 'opacity-30 pointer-events-none' : ''}`}>
        ...
        {aussagen.map((a, i) => {
          ...
          return (
            <div key={a.nr} className={`rounded-xl border p-3 transition-all ${
              wasWrong
