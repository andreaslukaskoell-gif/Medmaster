...
export function MRSWidget({ mrs, loading }: Props) {
    ...
    return (
      <Card className="border-dashed">
        <CardContent className="p-4 text-center space-y-1">
          ...
          <p className="text-xs text-muted-foreground">Beantworte 10+ Fragen um deinen Score zu sehen.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  const scoreLabel =
    mrs.score >= 80 ? 'Sehr gut 🟢' :
    mrs.score >= 60 ? 'Gut 🟡' :
    mrs.score >= 40 ? 'Ausbaufähig 🟠' :
    'Anfänger 🔴';
    ...
    <Card>
      <CardContent className="p-4">
        ...
        <div className="grid grid-cols-3 gap-2 text-center">
          ...
          <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-2">
            <p className="text-xs font-bold text-gray-900 dark:text-gray-100">
              {mrs.consistencyDays}<span className="font-normal text-[10px]">d</span>
            </p>
        ...
        {mrs.brierScore != null && (
          <p className="text-[10px] text-muted-foreground mt-2 text-center">
            ...
            <span className={mrs.brierScore < 0.1 ? 'text-green-500' : mrs.brierScore < 0.2 ? 'text-yellow-500' : 'text-red-500'}>
              {mrs.brierScore < 0.1 ? '(ausgezeichnet)' : mrs.brierScore < 0.2 ? '(gut)' : '(überkalibriert)'}
            </span>
          </p>
        )}
      </CardContent>
