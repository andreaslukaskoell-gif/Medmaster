    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-2 mb-2">
        <button
          ...
          className={`text-xs px-3 py-1 rounded-full border transition-colors ${
            showParallelogram ? "bg-teal-600 text-white border-teal-600" : "border-teal-300 text-teal-700 dark:text-teal-300 dark:border-teal-700"
          }`}
        >
      ...
      <svg viewBox="0 0 420 350" className="w-full max-w-lg mx-auto">
        ...
        {Array.from({ length: 7 }, (_, i) => (
          <g key={`gx-${i}`}>
            <line x1={toSvgX(i)} y1={toSvgY(0)} x2={toSvgX(i)} y2={toSvgY(6)} stroke="#f3f4f6" strokeWidth="1" />
          </g>
        ))}
        {Array.from({ length: 7 }, (_, i) => (
          <g key={`gy-${i}`}>
            <line x1={toSvgX(0)} y1={toSvgY(i)} x2={toSvgX(6)} y2={toSvgY(i)} stroke="#f3f4f6" strokeWidth="1" />
          </g>
        ))}
        ...
        {showParallelogram && (
          <>
            <line x1={toSvgX(aX)} y1={toSvgY(aY)} x2={toSvgX(sumX)} y2={toSvgY(sumY)} stroke="#ef4444" strokeWidth="1.5" strokeDasharray="5 3" opacity="0.5" />
            <line x1={toSvgX(bX)} y1={toSvgY(bY)} x2={toSvgX(sumX)} y2={toSvgY(sumY)} stroke="#3b82f6" strokeWidth="1.5" strokeDasharray="5 3" opacity="0.5" />
          </>
        )}
      ...
      <div className="bg-teal-50 dark:bg-teal-900/20 border border-teal-200 dark:border-teal-800 rounded-lg p-3">
        <p className="text-xs text-gray-700 dark:text-gray-300">
          Vektoren werden komponentenweise addiert: Die x- und y-Komponenten werden einzeln summiert.
          Graphisch entspricht die Summe der Diagonale des Parallelogramms, das von den beiden Vektoren aufgespannt wird.
        </p>
      </div>
    </div>
  );
