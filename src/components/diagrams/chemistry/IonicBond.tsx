...
export default function IonicBond() {
  ...
  return (
    <div className="space-y-3">
      <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Ionenbindung — NaCl</h4>
      <p className="text-xs text-muted">Elektronenuebertragung und Kristallgitter.</p>
      <div className="flex gap-2 mb-2">
        <button
          onClick={() => setStep("transfer")}
          className={`text-xs px-3 py-1 rounded-full border transition-colors ${
            step === "transfer" ? "bg-teal-600 text-white border-teal-600" : "border-teal-300 text-teal-700 dark:text-teal-300 dark:border-teal-700"
          }`}
        >
        ...
        <button
          ...
          className={`text-xs px-3 py-1 rounded-full border transition-colors ${
            step === "lattice" ? "bg-teal-600 text-white border-teal-600" : "border-teal-300 text-teal-700 dark:text-teal-300 dark:border-teal-700"
          }`}
        >
          Kristallgitter
        </button>
      ...
      {step === "transfer" ? (
        <svg viewBox="0 0 480 260" className="w-full max-w-lg mx-auto">
          <defs>
            ...
            <marker id="arrowTransfer" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto">
          ...
          <text x="127" y="113" textAnchor="middle" fontSize="6" fill="#fff" fontWeight="bold">e\u207B</text>
          ...
          {[0, 51, 103, 154, 206, 257, 309].map((angle, i) => {
            ...
            return <circle key={i} cx={ex} cy={ey} r="4" fill="#f59e0b" stroke="#d97706" strokeWidth="0.8" opacity="0.7" />;
          })}

          {/* Arrow down */}
          <text x="200" y="175" textAnchor="middle" fontSize="20" fill="#0d9488">\u2193</text>
          ...
          <line x1="150" y1="220" x2="244" y2="220" stroke="#0d9488" strokeWidth="1.5" strokeDasharray="4 3" />
            ...
            [0, 1, 2, 3, 4, 5].map((col) => {
              ...
              return (
                <g key={`${row}-${col}`}>
                  {/* Bonds to neighbors */}
                  {col < 5 && (
                    <line x1={cx + r} y1={cy} x2={cx + 50 - (isNa ? 18 : 14)} y2={cy} stroke="#d1d5db" strokeWidth="1" />
                  )}
                  {row < 5 && (
                    <line x1={cx} y1={cy + r} x2={cx} y2={cy + 40 - ((row + 1 + col) % 2 === 0 ? 14 : 18)} stroke="#d1d5db" strokeWidth="1" />
                  )}
                  <circle
                    cx={cx}
                    cy={cy}
                    ...
                    strokeWidth="1.5"
                  />
                  <text x={cx} y={cy + 3} textAnchor="middle" fontSize={isNa ? "7" : "7"} fontWeight="bold" fill={isNa ? "#991b1b" : "#15803d"}>
                    {isNa ? "Na\u207A" : "Cl\u207B"}
                  </text>
                </g>
              );
