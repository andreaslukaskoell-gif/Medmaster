      ...
      <svg viewBox="0 0 480 420" className="w-full max-w-xl mx-auto">
        ...
        <g>
          ...
          {[0, 45, 90, 135, 180, 225, 270, 315].map((angle) => {
            ...
            return (
              <line key={`spike-${angle}`}
                x1={240 + 18 * Math.cos(rad)} y1={30 + 18 * Math.sin(rad)}
                x2={240 + 26 * Math.cos(rad)} y2={30 + 26 * Math.sin(rad)}
                stroke="#ef4444" strokeWidth="2" strokeLinecap="round"
              />
            );
          })}
          <text x="240" y="34" textAnchor="middle" fontSize="7" fill="#dc2626" fontWeight="bold">Erreger</text>
        </g>
        ...
        {/* Timeline label */}
        <text x="130" y="145" textAnchor="middle" fontSize="7" fill="#0d9488">Minuten – Stunden</text>

        {/* Makrophagen */}
        <g className="cursor-pointer" onMouseEnter={() => setActive(parts[2])} onMouseLeave={() => setActive(null)}>
          <circle cx="70" cy="195" r="28" fill={active?.id === "makro" ? "#ccfbf1" : "#f0fdfa"} stroke="#0d9488" strokeWidth="2" />
          {/* Pseudopodia */}
          <path d="M45,185 Q38,178 42,172" fill="none" stroke="#0d9488" strokeWidth="2" strokeLinecap="round" />
          <path d="M50,210 Q42,218 46,225" fill="none" stroke="#0d9488" strokeWidth="2" strokeLinecap="round" />
          <path d="M92,180 Q100,174 98,168" fill="none" stroke="#0d9488" strokeWidth="2" strokeLinecap="round" />
          <text x="70" y="192" textAnchor="middle" fontSize="7" fill="#115e59" fontWeight="bold">Makro-</text>
          <text x="70" y="202" textAnchor="middle" fontSize="7" fill="#115e59" fontWeight="bold">phage</text>
          {/* Phagocytosis illustration */}
          <circle cx="70" cy="188" r="5" fill="#fecaca" stroke="#ef4444" strokeWidth="0.8" opacity="0.6" />
        </g>

        {/* NK-Zellen */}
        <g className="cursor-pointer" onMouseEnter={() => setActive(parts[3])} onMouseLeave={() => setActive(null)}>
          <circle cx="135" cy="195" r="24" fill={active?.id === "nk" ? "#ccfbf1" : "#f0fdfa"} stroke="#14b8a6" strokeWidth="2" />
          {/* Granules */}
          ...
          <text x="135" y="188" textAnchor="middle" fontSize="7" fill="#115e59" fontWeight="bold">NK-</text>
          <text x="135" y="198" textAnchor="middle" fontSize="7" fill="#115e59" fontWeight="bold">Zelle</text>
        </g>

        {/* Komplementsystem */}
        <g className="cursor-pointer" onMouseEnter={() => setActive(parts[4])} onMouseLeave={() => setActive(null)}>
          <rect x="170" y="172" width="60" height="46" rx="10" fill={active?.id === "komplement" ? "#ccfbf1" : "#f0fdfa"} stroke="#0f766e" strokeWidth="2" />
        ...
        <g className="cursor-pointer" onMouseEnter={() => setActive(parts[5])} onMouseLeave={() => setActive(null)}>
          <ellipse cx="240" cy="280" rx="50" ry="25" fill={active?.id === "dendritisch" ? "#ccfbf1" : "#f0fdfa"} stroke="#eab308" strokeWidth="2" />
          {/* Dendrites */}
          {[150, 180, 210, 270, 300, 330].map((angle) => {
            const rad = (angle * Math.PI) / 180;
            return (
              <line key={`dend-${angle}`}
                x1={240 + 48 * Math.cos(rad)} y1={280 + 23 * Math.sin(rad)}
                x2={240 + 62 * Math.cos(rad)} y2={280 + 32 * Math.sin(rad)}
                stroke="#eab308" strokeWidth="1.5" strokeLinecap="round"
              />
            );
          })}
          <text x="240" y="277" textAnchor="middle" fontSize="7" fill="#92400e" fontWeight="bold">Dendritische</text>
          <text x="240" y="287" textAnchor="middle" fontSize="7" fill="#92400e" fontWeight="bold">Zelle (APC)</text>
        </g>
        ...
        <text x="150" y="244" fontSize="6" fill="#eab308">Antigenpräsentation</text>
        ...
        {/* Timeline label */}
        <text x="360" y="145" textAnchor="middle" fontSize="7" fill="#0d9488">Tage – Wochen</text>

        {/* T-Zellen */}
        <g className="cursor-pointer" onMouseEnter={() => setActive(parts[6])} onMouseLeave={() => setActive(null)}>
          <circle cx="320" cy="195" r="26" fill={active?.id === "tcell" ? "#ccfbf1" : "#f0fdfa"} stroke="#6366f1" strokeWidth="2" />
          <text x="320" y="190" textAnchor="middle" fontSize="7" fill="#4f46e5" fontWeight="bold">T-Zelle</text>
          {/* TCR */}
          <path d="M307,178 Q310,168 314,178" fill="none" stroke="#6366f1" strokeWidth="1.5" />
          <text x="320" y="202" textAnchor="middle" fontSize="6" fill="#4f46e5">CD4⁺ / CD8⁺</text>
        </g>

        {/* B-Zellen */}
        <g className="cursor-pointer" onMouseEnter={() => setActive(parts[7])} onMouseLeave={() => setActive(null)}>
          <circle cx="410" cy="195" r="26" fill={active?.id === "bcell" ? "#ccfbf1" : "#f0fdfa"} stroke="#8b5cf6" strokeWidth="2" />
          <text x="410" y="190" textAnchor="middle" fontSize="7" fill="#7c3aed" fontWeight="bold">B-Zelle</text>
          {/* BCR / Y-shaped antibody */}
          <path d="M404,175 L400,165 M404,175 L408,165 M404,175 L404,180" stroke="#8b5cf6" strokeWidth="1.5" strokeLinecap="round" />
          <text x="410" y="202" textAnchor="middle" fontSize="6" fill="#7c3aed">→ Plasmazelle</text>
        </g>

        {/* Antibodies from B-cell */}
        <g className="cursor-pointer" onMouseEnter={() => setActive(parts[7])} onMouseLeave={() => setActive(null)}>
          {[{x: 430, y: 240}, {x: 450, y: 260}, {x: 440, y: 280}].map((pos, i) => (
            <g key={`ab-${i}`}>
              <path d={`M${pos.x},${pos.y + 8} L${pos.x - 4},${pos.y} M${pos.x},${pos.y + 8} L${pos.x + 4},${pos.y} M${pos.x},${pos.y + 8} L${pos.x},${pos.y + 14}`} stroke="#8b5cf6" strokeWidth="1.5" strokeLinecap="round" />
            </g>
          ))}
          <text x="445" y="300" fontSize="7" fill="#7c3aed" fontWeight="bold">Antikörper</text>
        </g>
        ...
        {/* Arrows to memory */}
        <line x1="320" y1="222" x2="335" y2="330" stroke="#0d9488" strokeWidth="1" strokeDasharray="3 2" />
        <line x1="410" y1="222" x2="375" y2="330" stroke="#0d9488" strokeWidth="1" strokeDasharray="3 2" />
        ...
        <text x="365" y="190" textAnchor="middle" fontSize="5" fill="#0d9488">Hilfe</text>
