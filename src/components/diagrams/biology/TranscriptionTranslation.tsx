    ...
    <div className="space-y-3">
      ...
      <svg viewBox="0 0 480 400" className="w-full max-w-xl mx-auto">
        ...
        <g className="cursor-pointer" onMouseEnter={() => setActive(parts[0])} onMouseLeave={() => setActive(null)}>
          ...
          <text x="30" y="65" fontSize="7" fill="#0f766e">3&apos;</text>
          <text x="445" y="65" fontSize="7" fill="#0f766e">5&apos;</text>
          {/* Coding strand 5'→3' */}
          <line x1="40" y1="80" x2="180" y2="80" stroke="#14b8a6" strokeWidth="3" />
          <line x1="320" y1="80" x2="440" y2="80" stroke="#14b8a6" strokeWidth="3" />
          <text x="30" y="85" fontSize="7" fill="#14b8a6">5&apos;</text>
          <text x="445" y="85" fontSize="7" fill="#14b8a6">3&apos;</text>
        ...
        {/* DNA labels */}
        <text x="100" y="58" textAnchor="middle" fontSize="7" fill="#0f766e">Matrizenstrang</text>
        <text x="100" y="96" textAnchor="middle" fontSize="7" fill="#14b8a6">Codogener Strang</text>

        {/* RNA Polymerase */}
        <g className="cursor-pointer" onMouseEnter={() => setActive(parts[1])} onMouseLeave={() => setActive(null)}>
          <ellipse cx="250" cy="72" rx="35" ry="20" fill="#6366f1" opacity="0.8" stroke="#4f46e5" strokeWidth="1.5" />
          <text x="250" y="69" textAnchor="middle" fontSize="7" fill="white" fontWeight="bold">RNA-</text>
          <text x="250" y="79" textAnchor="middle" fontSize="7" fill="white" fontWeight="bold">Polymerase</text>
        </g>

        {/* Growing mRNA */}
        <g className="cursor-pointer" onMouseEnter={() => setActive(parts[2])} onMouseLeave={() => setActive(null)}>
          <path d="M250,92 Q260,105 270,100 Q290,110 310,105 Q330,115 350,110 Q370,120 390,115" fill="none" stroke="#f97316" strokeWidth="2.5" strokeLinecap="round" />
          {/* Codons on mRNA */}
        ...
        <text x="253" y="54" textAnchor="middle" fontSize="6" fill="#0d9488">Richtung</text>

        {/* Arrow between sections */}
        <line x1="240" y1="176" x2="240" y2="196" stroke="#0d9488" strokeWidth="2" />
        <polygon points="236,196 240,204 244,196" fill="#0d9488" />
        <text x="260" y="193" fontSize="7" fill="#0d9488" fontWeight="bold">mRNA wandert zum Ribosom</text>
        ...
        {/* mRNA strand */}
        <g className="cursor-pointer" onMouseEnter={() => setActive(parts[2])} onMouseLeave={() => setActive(null)}>
          <line x1="50" y1="300" x2="430" y2="300" stroke="#f97316" strokeWidth="3" strokeLinecap="round" />
          <text x="40" y="296" fontSize="7" fill="#f97316">5&apos;</text>
          <text x="438" y="296" fontSize="7" fill="#f97316">3&apos;</text>
          {/* Codons */}
          {["A", "U", "G", "G", "C", "A", "U", "U", "C", "A", "G", "U", "C", "A", "A", "U", "A", "G"].map((b, i) => (
            <text key={`codon-${i}`} x={62 + i * 20} y={313} fontSize="7" fill="#f97316" fontWeight="bold">{b}</text>
          ))}
          {/* Codon brackets */}
          {[0, 1, 2, 3, 4, 5].map((g) => (
            <line key={`bracket-${g}`} x1={58 + g * 60} y1={316} x2={114 + g * 60} y2={316} stroke="#f97316" strokeWidth="0.8" opacity="0.5" />
          ))}
        </g>
        ...
        <g className="cursor-pointer" onMouseEnter={() => setActive(parts[3])} onMouseLeave={() => setActive(null)}>
          {/* Large subunit */}
          <ellipse cx="220" cy="285" rx="55" ry="25" fill="#99f6e4" opacity="0.5" stroke="#0d9488" strokeWidth="1.5" />
          <text x="220" y="278" textAnchor="middle" fontSize="7" fill="#115e59">Große UE</text>
          {/* Small subunit */}
        ...
        {[
          ...
          <g key={`trna-${i}`} className="cursor-pointer" onMouseEnter={() => setActive(parts[4])} onMouseLeave={() => setActive(null)}>
            {/* tRNA body */}
            <path d={`M${trna.x},${295} L${trna.x - 8},${265} Q${trna.x},${255} ${trna.x + 8},${265} Z`} fill={trna.color} opacity="0.6" stroke={trna.color} strokeWidth="1" />
            {/* Anticodon label */}
            <text x={trna.x} y={292} textAnchor="middle" fontSize="5" fill={trna.color} fontWeight="bold">{trna.codon}</text>
            {/* Amino acid */}
            <circle cx={trna.x} cy={253} r="8" fill={trna.color} opacity="0.3" stroke={trna.color} strokeWidth="1" />
            <text x={trna.x} y={256} textAnchor="middle" fontSize="6" fill={trna.color} fontWeight="bold">{trna.aa}</text>
          </g>
        ))}

        {/* tRNA label */}
        <text x="280" y="250" fontSize="7" fill="#6366f1" fontWeight="bold">tRNA</text>
        ...
        <g className="cursor-pointer" onMouseEnter={() => setActive(parts[5])} onMouseLeave={() => setActive(null)}>
          <path d="M192,253 Q175,240 160,248 Q145,256 130,248 Q115,240 100,248 Q85,256 75,248" fill="none" stroke="#ef4444" strokeWidth="3" strokeLinecap="round" />
          {[160, 130, 100].map((px, i) => (
            <circle key={`aa-${i}`} cx={px} cy={248} r="6" fill="#fecaca" stroke="#ef4444" strokeWidth="1" />
          ))}
          <text x="110" y="238" textAnchor="middle" fontSize="7" fill="#ef4444" fontWeight="bold">Polypeptidkette</text>
        </g>

        {/* Direction arrow */}
        <line x1="165" y1="350" x2="275" y2="350" stroke="#0d9488" strokeWidth="1" />
        <polygon points="275,347 281,350 275,353" fill="#0d9488" />
        <text x="220" y="362" textAnchor="middle" fontSize="7" fill="#0d9488">Leserichtung 5&apos;→3&apos;</text>

        {/* A, P, E site labels */}
        <text x="248" y="288" textAnchor="middle" fontSize="6" fill="#115e59" fontWeight="bold">A</text>
        <text x="220" y="288" textAnchor="middle" fontSize="6" fill="#115e59" fontWeight="bold">P</text>
        <text x="192" y="288" textAnchor="middle" fontSize="6" fill="#115e59" fontWeight="bold">E</text>
        <text x="340" y="265" fontSize="6" fill="#115e59">A = Aminoacyl-Stelle</text>
        <text x="340" y="275" fontSize="6" fill="#115e59">P = Peptidyl-Stelle</text>
        <text x="340" y="285" fontSize="6" fill="#115e59">E = Exit-Stelle</text>
      </svg>
