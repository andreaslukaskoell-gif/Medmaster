...
export default function MenstrualCycle() {
  ...
  const toPath = (data: number[][]) =>
    data
      .map(([d, v], i) => `${i === 0 ? "M" : "L"}${dayToX(d).toFixed(1)},${valToY(v).toFixed(1)}`)
      .join(" ");
  ...
  const getDayInfo = (day: number): string => {
    if (day <= 5)
      return "Menstruation: Endometrium wird abgestossen. Östrogen und Progesteron niedrig.";
    if (day <= 13)
      return "Follikelphase: FSH stimuliert Follikelreifung. Östrogen steigt → Endometriumaufbau.";
    if (day === 14) return "Ovulation: LH-Peak löst Eisprung aus! Reife Eizelle wird freigesetzt.";
    if (day <= 28)
      return "Lutealphase: Gelbkörper produziert Progesteron. Endometrium wird für Einnistung vorbereitet.";
    return "";
  };
  ...
  return (
    <div className="space-y-3">
      <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100">
        Menstruationszyklus — Hormonspiegel
      </h4>
      ...
      <svg viewBox="0 0 480 340" className="w-full max-w-xl mx-auto">
        ...
        {phases.map((phase) => (
          <g key={phase.label}>
            <rect
              x={dayToX(phase.start)}
              y={gT}
              width={dayToX(phase.end) - dayToX(phase.start)}
              height={gH}
              fill={phase.color}
              opacity="0.4"
            />
            <text
              x={(dayToX(phase.start) + dayToX(phase.end)) / 2}
              y={gB + 40}
              textAnchor="middle"
              fontSize="7"
              fill="#115e59"
        ...
        {Array.from({ length: 28 }, (_, i) => i + 1).map((d) => (
          <g key={`tick-${d}`}>
            ...
            />
            {d % 2 === 0 && (
              <text x={dayToX(d)} y={gB + 14} textAnchor="middle" fontSize="6" fill="#115e59">
                {d}
              </text>
            )}
          </g>
        ...
        <text
          ...
          fill="#0d9488"
          fontWeight="bold"
          transform={`rotate(-90, 15, ${(gT + gB) / 2})`}
        >
          Hormonspiegel
        </text>
        ...
        {/* LH peak label */}
        <text x={dayToX(14) + 5} y={valToY(0.95) + 4} fontSize="7" fill="#16a34a" fontWeight="bold">
          LH-Peak
        </text>
        ...
        {hormones.map((h, i) => (
          <g key={`legend-${h.name}`} transform={`translate(${gL + i * 100}, 310)`}>
            <line
              ...
              strokeLinecap="round"
            />
            <text x="25" y="4" fontSize="13" fill={h.color} fontWeight="bold">
              {h.name}
            </text>
          </g>
        ))}
