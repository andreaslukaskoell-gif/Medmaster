...
const parts = [
  { id: "z-line", label: "Z-Scheibe", detail: "Begrenzung des Sarkomers, verankert Aktinfilamente. Besteht aus α-Actinin." },
  { id: "actin", label: "Aktinfilament (dünnes Filament)", detail: "Besteht aus F-Aktin mit Troponin und Tropomyosin. Bindet Myosinköpfe während Kontraktion." },
  { id: "myosin", label: "Myosinfilament (dickes Filament)", detail: "Motor-Protein mit beweglichen Köpfen (Querbrücken), spaltet ATP für Kontraktion." },
  { id: "i-band", label: "I-Bande (isotrop)", detail: "Helle Zone mit nur Aktinfilamenten. Verkürzt sich bei Kontraktion." },
  { id: "a-band", label: "A-Bande (anisotrop)", detail: "Dunkle Zone über gesamte Länge der Myosinfilamente. Länge bleibt konstant." },
  { id: "h-zone", label: "H-Zone", detail: "Zentrum mit nur Myosinfilamenten, keine Überlappung. Verschwindet fast bei maximaler Kontraktion." },
  { id: "m-line", label: "M-Linie", detail: "Mittelstruktur des Sarkomers, hält Myosinfilamente in Position." },
  { id: "titin", label: "Titin", detail: "Elastisches Riesenprotein, verbindet Z-Scheibe mit Myosin, stabilisiert und verhindert Überdehnung." },
  { id: "troponin", label: "Troponin + Tropomyosin", detail: "Regulatorische Proteine auf Aktin. Ca²⁺ bindet an Troponin → Freigabe der Myosin-Bindungsstellen." },
];
...
export default function Sarcomere() {
  ...
  return (
    <div className="space-y-3">
      <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Sarkomer — Muskelkontraktion</h4>
      <p className="text-xs text-gray-600 dark:text-gray-400">Klicke auf Strukturen für Details zur Gleitfilamenttheorie.</p>
      <svg viewBox="0 0 600 420" className="w-full max-w-2xl mx-auto">
        ...
        {/* Title */}
        <text x="300" y="20" fontSize="14" fontWeight="600" textAnchor="middle" className="fill-gray-900 dark:fill-gray-100">
          Gleitfilamenttheorie der Muskelkontraktion
        </text>

        {/* ============ RELAXED STATE (TOP) ============ */}
        <g id="relaxed-state">
          <text x="10" y="50" fontSize="12" fontWeight="600" className="fill-gray-700 dark:fill-gray-300">
            Relaxierter Zustand
          </text>
          ...
          {/* M-line */}
          <line
            x1="300" y1="65" x2="300" y2="135"
            stroke="#6b7280" strokeWidth="2" strokeDasharray="3,3"
            cursor="pointer"
          ...
          {/* Left side actin */}
          <g cursor="pointer" onMouseEnter={() => setActive(parts.find(p => p.id === "actin") || null)} onMouseLeave={() => setActive(null)}>
          ...
          <g cursor="pointer" onMouseEnter={() => setActive(parts.find(p => p.id === "myosin") || null)} onMouseLeave={() => setActive(null)}>
          ...
          <g cursor="pointer" onMouseEnter={() => setActive(parts.find(p => p.id === "titin") || null)} onMouseLeave={() => setActive(null)}>
            <path
              d="M 52 100 Q 70 95, 88 100 T 124 100 T 160 100 T 180 100"
              stroke="#10b981" strokeWidth="2" fill="none" strokeDasharray="2,2"
            />
            <path
          ...
          <g cursor="pointer" onMouseEnter={() => setActive(parts.find(p => p.id === "troponin") || null)} onMouseLeave={() => setActive(null)}>
          ...
          {/* I-band label */}
          <g cursor="pointer" onMouseEnter={() => setActive(parts.find(p => p.id === "i-band") || null)} onMouseLeave={() => setActive(null)}>
            <line x1="80" y1="145" x2="80" y2="155" stroke="#94a3b8" strokeDasharray="2,2" />
            <text x="80" y="165" fontSize="9" textAnchor="middle" className="fill-gray-600 dark:fill-gray-400">I-Bande</text>
          </g>

          {/* A-band label */}
          <g cursor="pointer" onMouseEnter={() => setActive(parts.find(p => p.id === "a-band") || null)} onMouseLeave={() => setActive(null)}>
            <line x1="300" y1="145" x2="300" y2="155" stroke="#94a3b8" strokeDasharray="2,2" />
            <text x="300" y="165" fontSize="9" textAnchor="middle" className="fill-gray-600 dark:fill-gray-400">A-Bande</text>
          </g>

          {/* H-zone label */}
          <g cursor="pointer" onMouseEnter={() => setActive(parts.find(p => p.id === "h-zone") || null)} onMouseLeave={() => setActive(null)}>
            <line x1="300" y1="60" x2="300" y2="50" stroke="#94a3b8" strokeDasharray="2,2" />
            <text x="300" y="45" fontSize="9" textAnchor="middle" className="fill-gray-600 dark:fill-gray-400">H-Zone</text>
          </g>

          {/* Sarcomere bracket */}
          <path d="M 50 145 L 50 150 L 550 150 L 550 145" stroke="#1f2937" strokeWidth="1.5" fill="none" />
          <text x="300" y="180" fontSize="10" textAnchor="middle" className="fill-gray-700 dark:fill-gray-300">Sarkomer (≈2.2 μm)</text>
        </g>

        {/* ============ CONTRACTED STATE (BOTTOM) ============ */}
        <g id="contracted-state">
          <text x="10" y="215" fontSize="12" fontWeight="600" className="fill-gray-700 dark:fill-gray-300">
            Kontrahierter Zustand
          </text>

          {/* Contraction arrows */}
          <path d="M 120 200 L 140 200" stroke="#dc2626" strokeWidth="2" markerEnd="url(#arrowRed)" />
          <path d="M 480 200 L 460 200" stroke="#dc2626" strokeWidth="2" markerEnd="url(#arrowRed)" />
          <defs>
          ...
          <g cursor="pointer" onMouseEnter={() => setActive(parts.find(p => p.id === "i-band") || null)} onMouseLeave={() => setActive(null)}>
            ...
            <text x="130" y="330" fontSize="9" textAnchor="middle" className="fill-gray-600 dark:fill-gray-400">I-Bande</text>
          </g>

          <g cursor="pointer" onMouseEnter={() => setActive(parts.find(p => p.id === "a-band") || null)} onMouseLeave={() => setActive(null)}>
            <line x1="300" y1="310" x2="300" y2="320" stroke="#94a3b8" strokeDasharray="2,2" />
            <text x="300" y="330" fontSize="9" textAnchor="middle" className="fill-gray-600 dark:fill-gray-400">A-Bande</text>
          </g>

          <g cursor="pointer" onMouseEnter={() => setActive(parts.find(p => p.id === "h-zone") || null)} onMouseLeave={() => setActive(null)}>
            <line x1="300" y1="225" x2="300" y2="215" stroke="#94a3b8" strokeDasharray="2,2" />
            <text x="300" y="210" fontSize="9" textAnchor="middle" className="fill-gray-600 dark:fill-gray-400">H-Zone</text>
          </g>

          {/* Sarcomere bracket (shorter) */}
          <path d="M 110 310 L 110 315 L 490 315 L 490 310" stroke="#1f2937" strokeWidth="1.5" fill="none" />
          <text x="300" y="345" fontSize="10" textAnchor="middle" className="fill-gray-700 dark:fill-gray-300">Sarkomer (≈1.5 μm)</text>
        </g>

        {/* Legend */}
        <g id="legend" transform="translate(10, 365)">
          <text x="0" y="0" fontSize="9" fontWeight="600" className="fill-gray-700 dark:fill-gray-300">Legende:</text>
          <rect x="0" y="5" width="20" height="4" fill="url(#actinGrad)" rx="1" />
          <text x="25" y="10" fontSize="8" className="fill-gray-600 dark:fill-gray-400">Aktin</text>

          <rect x="70" y="5" width="20" height="4" fill="url(#myosinGrad)" rx="1" />
          <text x="95" y="10" fontSize="8" className="fill-gray-600 dark:fill-gray-400">Myosin</text>

          <line x1="140" y1="7" x2="160" y2="7" stroke="#10b981" strokeWidth="2" strokeDasharray="2,2" />
          <text x="165" y="10" fontSize="8" className="fill-gray-600 dark:fill-gray-400">Titin</text>

          <circle cx="210" cy="7" r="2.5" fill="#f59e0b" />
          <text x="220" y="10" fontSize="8" className="fill-gray-600 dark:fill-gray-400">Troponin</text>

          <rect x="280" y="5" width="4" height="8" fill="#1f2937" />
          <text x="290" y="10" fontSize="8" className="fill-gray-600 dark:fill-gray-400">Z-Scheibe</text>
        </g>

        {/* Mechanism note */}
        <text x="300" y="395" fontSize="8" textAnchor="middle" className="fill-gray-500 dark:fill-gray-500" fontStyle="italic">
          Bei Kontraktion: Ca²⁺ → Troponin → Freigabe Bindungsstellen → Myosin zieht Aktin → Sarkomer verkürzt sich
        </text>
      </svg>
      {active && (
