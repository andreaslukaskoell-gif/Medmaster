...
export default function ActionPotential() {
  ...
  const curvePoints: [number, number][] = [
    [0, -70], [1.0, -70], [1.3, -55], [1.8, 0], [2.0, 30],
    [2.3, 20], [2.6, 0], [3.0, -50], [3.3, -70], [3.6, -80],
    [4.0, -75], [4.5, -70], [6, -70],
  ];
    ...
    <div className="space-y-3">
      ...
      <svg viewBox="0 0 480 380" className="w-full max-w-xl mx-auto">
        ...
        {[-90, -70, -55, -30, 0, 30].map((mv) => (
          <g key={`grid-${mv}`}>
            <line x1={graphL} y1={mvToY(mv)} x2={graphR} y2={mvToY(mv)} stroke="#99f6e4" strokeWidth="0.5" strokeDasharray="4 3" />
            <text x={graphL - 5} y={mvToY(mv) + 3} textAnchor="end" fontSize="7" fill="#115e59">{mv} mV</text>
          </g>
        ))}
        ...
        {[0, 1, 2, 3, 4, 5, 6].map((ms) => (
          <g key={`t-${ms}`}>
            <line x1={msToX(ms)} y1={graphB} x2={msToX(ms)} y2={graphB + 5} stroke="#115e59" strokeWidth="0.8" />
            <text x={msToX(ms)} y={graphB + 15} textAnchor="middle" fontSize="7" fill="#115e59">{ms} ms</text>
          </g>
        ))}
        ...
        <text x={graphR + 5} y={mvToY(-55) + 3} fontSize="7" fill="#f97316">Schwelle</text>

        {/* Resting potential line */}
        <line x1={graphL} y1={mvToY(-70)} x2={graphR} y2={mvToY(-70)} stroke="#6366f1" strokeWidth="1" strokeDasharray="6 3" opacity="0.5" />
        ...
        {/* Resting */}
        <g className="cursor-pointer" onMouseEnter={() => setActive(phases[0])} onMouseLeave={() => setActive(null)}>
          <rect x={msToX(0)} y={graphT} width={msToX(1.0) - msToX(0)} height={graphH} fill="#6366f1" opacity={active?.id === "resting" ? 0.15 : 0.05} />
          <text x={(msToX(0) + msToX(1.0)) / 2} y={graphT + 15} textAnchor="middle" fontSize="7" fill="#6366f1" fontWeight="bold">Ruhe</text>
        </g>

        {/* Depolarisation */}
        <g className="cursor-pointer" onMouseEnter={() => setActive(phases[2])} onMouseLeave={() => setActive(null)}>
          <rect x={msToX(1.0)} y={graphT} width={msToX(2.0) - msToX(1.0)} height={graphH} fill="#ef4444" opacity={active?.id === "depol" ? 0.15 : 0.05} />
          <text x={(msToX(1.0) + msToX(2.0)) / 2} y={graphT + 15} textAnchor="middle" fontSize="7" fill="#ef4444" fontWeight="bold">Depolarisation</text>
          <text x={(msToX(1.0) + msToX(2.0)) / 2} y={graphT + 26} textAnchor="middle" fontSize="6" fill="#ef4444">Na⁺ Einstrom</text>
        </g>

        {/* Overshoot marker */}
        <g className="cursor-pointer" onMouseEnter={() => setActive(phases[3])} onMouseLeave={() => setActive(null)}>
          <circle cx={msToX(2.0)} cy={mvToY(30)} r="6" fill={active?.id === "overshoot" ? "#ccfbf1" : "#f0fdfa"} stroke="#0d9488" strokeWidth="1.5" />
          <text x={msToX(2.0) + 10} y={mvToY(30) - 5} fontSize="7" fill="#0d9488" fontWeight="bold">+30 mV</text>
          <text x={msToX(2.0) + 10} y={mvToY(30) + 5} fontSize="6" fill="#0d9488">Overshoot</text>
        </g>

        {/* Repolarisation */}
        <g className="cursor-pointer" onMouseEnter={() => setActive(phases[4])} onMouseLeave={() => setActive(null)}>
          <rect x={msToX(2.0)} y={graphT} width={msToX(3.3) - msToX(2.0)} height={graphH} fill="#14b8a6" opacity={active?.id === "repol" ? 0.15 : 0.05} />
          <text x={(msToX(2.0) + msToX(3.3)) / 2} y={graphT + 15} textAnchor="middle" fontSize="7" fill="#0f766e" fontWeight="bold">Repolarisation</text>
          <text x={(msToX(2.0) + msToX(3.3)) / 2} y={graphT + 26} textAnchor="middle" fontSize="6" fill="#0f766e">K⁺ Ausstrom</text>
        </g>

        {/* Hyperpolarisation */}
        <g className="cursor-pointer" onMouseEnter={() => setActive(phases[5])} onMouseLeave={() => setActive(null)}>
          <rect x={msToX(3.3)} y={graphT} width={msToX(4.5) - msToX(3.3)} height={graphH} fill="#8b5cf6" opacity={active?.id === "hyper" ? 0.15 : 0.05} />
          <text x={(msToX(3.3) + msToX(4.5)) / 2} y={mvToY(-80) - 8} textAnchor="middle" fontSize="6" fill="#8b5cf6" fontWeight="bold">Hyperpol.</text>
          <text x={(msToX(3.3) + msToX(4.5)) / 2} y={mvToY(-80) + 4} textAnchor="middle" fontSize="6" fill="#8b5cf6">-80 mV</text>
        </g>

        {/* Threshold marker */}
        <g className="cursor-pointer" onMouseEnter={() => setActive(phases[1])} onMouseLeave={() => setActive(null)}>
          <circle cx={msToX(1.3)} cy={mvToY(-55)} r="5" fill={active?.id === "threshold" ? "#fef3c7" : "#fff7ed"} stroke="#f97316" strokeWidth="1.5" />
        </g>
