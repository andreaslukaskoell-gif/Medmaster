  ...
  try {
    ...
    for (const chapter of chapters) {
      try {
        ...
        const { error: chapterError } = await client.from("bms_chapters").upsert(chapterData, {
          onConflict: "id",
        });
        ...
        console.log(
          `✅ [migrateBMS] Migrated chapter ${chapter.title} with ${chapter.unterkapitel?.length || 0} subchapters`
        );
      } catch (chapterErr) {
...
export async function checkMigrationStatus(): Promise<{
  ...
  if (!client) {
    return {
      localStorageCount: localStorageChapters.length,
      supabaseCount: 0,
      needsMigration: false,
    };
  }
  try {
    const { data, error } = await client.from("bms_chapters").select("id", { count: "exact" });
